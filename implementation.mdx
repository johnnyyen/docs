...
To provide the best support for your customers, information about MNO availability is also published on our [dashboard](http://localhost:3000/dashboard/status) and [status page](https://status.pawapay.cloud).
...
All MNOs that you transact with through pawaPay have transaction limits. 
...
For example, a periodic job that collects all payments that have been pending for an extended period. It can then query the respective `Get status and details` endpoint to verify the status of the payment. 
