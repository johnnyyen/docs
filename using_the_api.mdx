...
## Where is the API?
    ...
    When [creating an account](https://dashboard.sandbox.pawapay.cloud/#/merchant-signup) with pawaPay, you will first receive access to our sandbox environemnt. 
...
__Do not forget to enable signed financial calls and upload your public key in the pawaPay Dashboard. Learn how to do that from the [pawaPay Dashboard documentation](/dashboard/other/system_conf/api_tokens#signed-requests).__

#### Make the request

You can now send this request to pawaPay Merchant API to initiate a [deposit](/v1/api-reference/deposits/request-deposit), [payout](/v1/api-reference/payouts/request-payout), [bulk payout](/v1/api-reference/payouts/request-bulk-payouts) and [refund](#operation/createRefund).
...
The parameters that define the destination of the payment are <code>correspondent</code> and <code>payer</code> for [Deposits](/v1/api-reference/deposits/request-deposit) or <code>recipient</code> for [Payouts](/v1/api-reference/payouts/request-payout). 
