...
After receiving a payout request, we send it to the MNO specified as the [correspondent](/using_the_api#correspondents). 
...
Sometimes, MNOs experience degraded performance or their systems are completely unavailable.
Read more about [MNO stability](/implementation#mno-stability).

Our Payment Operations team monitors the MNO availability 24/7 and publishes that information on our [status page](https://status.pawapay.cloud/). 
...
When an MNO cannot process payouts, our platform will still accept payout requests for processing. 
Your payout request will return 'ENQUEUED' as the status.
The payout will be enqueued until the MNO's systems become operational.
...
It is also possible to cancel it from the pawaPay Dashboard, as described in the [pawaPay Dashboard documentation](/dashboard/transactions/cancel_enqueued).
